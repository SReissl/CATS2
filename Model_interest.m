...
function  [Y_real,prob_DD_b,noprod,cactual,cdemand1,cdemand2,welfare_cs,welfare_dc1s,welfare_dc2s,welfare_c,welfare_dc1,welfare_dc2,Ygap,rDIV,rDIVK,dincome,dincomek,DIV,DIVK,OCC,wincome,deviation,opt_share,neut_share,pes_share,types_agg,gperiod,Growth,Dek,Dek2,Kdem2,bankruptcy_rate,residualC,YP_e,YP_u,prob_EE,prob_EU,prob_UU,prob_UE,prob_EE_p,prob_EU_p,prob_UU_p,prob_UE_p,prob_DD,prob_DN,prob_NN,prob_ND,prob_DD_k,prob_DN_k,prob_NN_k,prob_ND_k,price,Div_prob,Div_probb,Div_probk,profitsB,Cdivs_p,Kdivs_p,shares,dividendsB,EXPcontrol,Invent,Assets,baryk,Kdes,Kdem,Dexp,Kdivs,Cdivs,valI,unsatisfiedDemandK,actualEXP,X1,X2,totK, gdp_deflator, Investment,I, consumption, Prod_k, Prod_c, Un, totalDeb, totalDeb_k,stock_bonds,GB,TA,G,wages_t, DC,totE,average_interest_rate,impulse_rw_opt,YPE_true,YPE_anchor,YPU_true,YPU_anchor,rwage,Occ_status,Type,r_f,bond_interest_rate,et] = Model_interest(seed, T, par,Hor)
tic 
...
burnin=horizon+1;
r_f=zeros(1,T+1);
r_f(1)=0.01;%   0.015;% 0.005          %general refinancing rate
r_d=zeros(1,T+1);
r_d(1)=r_f(1)/2;                          %deposit rate
counter=0;
Type=zeros(T,W+F+N);
adapt=0.95;
opt_share=zeros(1,T);
...
bond_interest_rate=zeros(1,T+1);
bondrate_new=zeros(1,T+1);
bond_interest_rate(1) = r_f(1);   %% ricordare di aggiungere anche questi con liquidità
...
%%%%%%%%%%%%%%%%%%
 for t=2:T
     %disp(t)
     inflation=(price(t)-price(t-1))/price(t-1)-0.005;
     
     if MP==1 && t>500
        r_f(t)=max(0.001,0.5*r_f(t-1)+0.5*(0.01+inflation));
     else
        r_f(t)=r_f(t-1);
     end
     r_d(t)=r_f(t)/2;
...
%% CREDIT MARKET OPENS
    ...
    proposed_rate=mu*((1+r_f(t)/theta)./Xi - theta)';
    proposed_rate_k=mu*((1+r_f(t)/theta)./Xi_k - theta)';
...
%% UNEMPLOYED WORKERS LOOK FOR A JOB
    ...
    interest_deposits=sum(r_d(t)*PA(1:W));
    PA(1:W)=PA(1:W)+r_d(t)*PA(1:W);
    if YPtest==0
        PA(1:W)=PA(1:W)+wn;
    end
...
if YPtest == 1 
    ...
    permw=projectIncome(prob_EE(t),prob_EU(t),prob_UE(t),prob_UU(t),pay1,pay2,r_d(t));
    
    YPE(:,t)=(PA(1:W)/price(t)+permw(1))/(1/r_d(t));
    YPU(:,t)=(PA(1:W)/price(t)+permw(2))/(1/r_d(t));
    YPE_belief(:,t)=(permw(1))/(1/r_d(t));
    ...
    if opinions==1
        if t>=shockperiod && t< shockperiod+duration
            ...
            permwopt=projectIncome(probs11,probs12,probs21,probs22,pay1_opt,pay2_opt,r_d(t));
            pay1_pes=pay1;
            pay2_pes=pay2;
            ...
            permwpes=projectIncome(probs11,probs12,probs21,probs22,pay1_pes,pay2_pes,r_d(t));
            
            for i=1:W
            if types(i)==1
            YPE(i,t)=(PA(i)/price(t)+permwopt(1))/(1/r_d(t));
            YPU(i,t)=(PA(i)/price(t)+permwopt(2))/(1/r_d(t));
            YPE_belief(i,t)=(permwopt(1))/(1/r_d(t));
            end
            if types(i)==-1
            YPE(i,t)=(PA(i)/price(t)+permwpes(1))/(1/r_d(t));
            YPU(i,t)=(PA(i)/price(t)+permwpes(2))/(1/r_d(t));
            YPE_belief(i,t)=(permwpes(1))/(1/r_d(t));
            end
            end
        end
    end
    ...
    if t>=shockperiod && opinions==1 && switching==1
       ...
       permwt=projectIncome(prob_EE_p(t),prob_EU_p(t),prob_UE_p(t),prob_UU_p(t),pay1_true,pay2_true,r_d(t));
       YPE_true(t)=permwt(1)/(1/r_d(t));
       for i=1:W
...
%%ACCOUNTING
    ...
    interest_deposits=interest_deposits+sum(r_d(t)*PA(W+1:W+F));
    PA(W+1:W+F)=PA(W+1:W+F)+r_d(t)*PA(W+1:W+F);
    ...
    interest_deposits=interest_deposits+sum(r_d(t)*PA(W+F+1:W+F+N));
    PA(W+F+1:W+F+N)=PA(W+F+1:W+F+N)+r_d(t)*PA(W+F+1:W+F+N);
...
%          display(seed)
    ...
    %% bank accounting
    piB=piB+sum(interests)+sum(interests_k) + bond_interest_rate(t-1)*bonds(t-1)-interest_deposits;                                             %bank profits  
 ...
 GB(t)= TA(t) - G(t)- EXP(t)- bond_interest_rate(t-1)*bonds(t-1); %JAKOB bonds(t-1) mi impalla tutto ma non potevo
 primary_GB(t) = TA(t) - G(t)- EXP(t);

 stock_bonds(t) =sum(-GB(1:t));
 bonds(t) = max(0,stock_bonds(t));
 bondrate_new(t)=((bonds(t)/E-5.5))/75;
 if end_bondrate==1 && t>500
    bond_interest_rate(t+1)=max(0,bondrate_new(t));
 else
    bond_interest_rate(t+1)=r_f(t);
 end
...
if YPtest==1
    ...
    if t < burnin
        ...
        permdb=projectIncome(prob_DD_b(t),prob_DN_b(t),prob_ND_b(t),prob_NN_b(t),pay1b,pay2b,r_d(t));
        ...
        if opinions==1
            ...
            permdbopt=projectIncome(probs11,probs12,probs21,probs22,pay1b_opt,pay2b_opt,r_d(t));
            pay1b_pes=pay1b;
            pay2b_pes=pay2b;
            ...
            permdbpes=projectIncome(probs11,probs12,probs21,probs22,pay1b_pes,pay2b_pes,r_d(t));
            end
        end
        ...
        permds=projectIncomeMult(prob_DD(t),prob_DN(t),prob_ND(t),prob_NN(t),pay1,pay2,r_d(t));
        for i=1:F
        permd=permds(:,i);
        V_div=permd;
            if dividends_income(i)>0
                ...
                yd(i)=(PA(W+i)/price(t)+V_div)/(1/r_d(t));
                yd_belief(i,t)=(V_div)/(1/r_d(t));
            else
                if dividendsB(t)>0
    ...
    if opinions==1 && t>=shockperiod && t<shockperiod+duration
        ...
        permds_opt=projectIncomeMult(probs11opt,probs12opt,probs21opt,probs22opt,pay1_e,pay2_e,r_d(t));
        permds_pes=projectIncomeMult(probs11pes,probs12pes,probs21pes,probs22pes,pay1_e,pay2_e,r_d(t));
        for i=1:F
        if types(W+i)==1
            V_div=permds_opt(:,i);
            if dividendsB(t)>0
                if dividends_income(i)>0
                    yd(i)=(PA(W+i)/price(t)+V_div(1)+permdbopt(1))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(1)+permdbopt(1))/(1/r_d(t));
                else
                    yd(i)=(PA(W+i)/price(t)+V_div(2)+permdbopt(1))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(2)+permdbopt(1))/(1/r_d(t));
                end
            else
                if dividends_income(i)>0
                    yd(i)=(PA(W+i)/price(t)+V_div(1)+permdbopt(2))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(1)+permdbopt(2))/(1/r_d(t));
                else
                    yd(i)=(PA(W+i)/price(t)+V_div(2)+permdbopt(2))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(2)+permdbopt(2))/(1/r_d(t));
                end
            end
        end
        if types(W+i)==-1
            ...
            if dividendsB(t)>0
                if dividends_income(i)>0
                    yd(i)=(PA(W+i)/price(t)+V_div(1)+permdbpes(1))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(1)+permdbpes(1))/(1/r_d(t));
                else
                    yd(i)=(PA(W+i)/price(t)+V_div(2)+permdbpes(1))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(2)+permdbpes(1))/(1/r_d(t));
                end
            else
                if dividends_income(i)>0
                    yd(i)=(PA(W+i)/price(t)+V_div(1)+permdbpes(2))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(1)+permdbpes(2))/(1/r_d(t));
                else
                    yd(i)=(PA(W+i)/price(t)+V_div(2)+permdbpes(2))/(1/r_d(t));
                    yd_belief(i,t)=(V_div(2)+permdbpes(2))/(1/r_d(t));
                end
            end
        end
        end
    ...
    if t>=shockperiod && switching==1
        if ~isinf(cond(tMatB_true))
           ...
           permdbt=projectIncome(prob_DD_bp(t),prob_DN_bp(t),prob_ND_bp(t),prob_NN_bp(t),pay1b_true,pay2b_true,r_d(t));
        end
        ...
        permdts=projectIncomeMult(prob_DD_p(t),prob_DN_p(t),prob_ND_p(t),prob_NN_p(t),pay1_true,pay2_true,r_d(t)); 
        end
        ...
        if dividends_income(i)>0
            if dividendsB(t)>0    
                yd_true=(V_div_true(1)+permdbt(1))/(1/r_d(t));
            else
                yd_true=(V_div_true(1)+permdbt(2))/(1/r_d(t));
            end
        else
            if dividendsB(t)>0    
                yd_true=(V_div_true(2)+permdbt(1))/(1/r_d(t));
            else
                yd_true=(V_div_true(2)+permdbt(2))/(1/r_d(t));
            end  
        end
    ...
    if t < burnin
       ...
       for l=1:N
        ...
        permdks=projectIncomeMult(prob_DD_k(t),prob_DN_k(t),prob_ND_k(t),prob_NN_k(t),pay1,pay2,r_d(t));
        for i=1:N
        permdk=permdks(:,i);
        V_divk=permdk;
            if dividends_income_k(i)>0
                ...
                ydk(i)=(PA(W+F+i)/price(t)+V_divk)/(1/r_d(t));
                ydk_belief(i,t)=(V_divk)/(1/r_d(t));
            else
                if dividendsB(t)>0
    ...
    if opinions==1 && t>=shockperiod && t<shockperiod+duration
        ...
        permdks_opt=projectIncomeMult(probs11opt,probs12opt,probs21opt,probs22opt,pay1_e,pay2_e,r_d(t));
        permdks_pes=projectIncomeMult(probs11pes,probs12pes,probs21pes,probs22pes,pay1_e,pay2_e,r_d(t));
        for l=1:N
        if types(W+F+l)==1
            V_divk=permdks_opt(:,l);
            if dividends_income_k(l)>0
                if dividendsB(t)>0
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(1)+permdbopt(1))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(1)+permdbopt(1))/(1/r_d(t));
                else
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(1)+permdbopt(2))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(1)+permdbopt(2))/(1/r_d(t));
                end
            else
                if dividendsB(t)>0
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(2)+permdbopt(1))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(2)+permdbopt(1))/(1/r_d(t));
                else
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(2)+permdbopt(2))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(2)+permdbopt(2))/(1/r_d(t));
                end
            end
        end
        if types(W+F+l)==-1
            ...
            if dividends_income_k(l)>0
                if dividendsB(t)>0
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(1)+permdbpes(1))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(1)+permdbpes(1))/(1/r_d(t));
                else
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(1)+permdbpes(2))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(1)+permdbpes(2))/(1/r_d(t));
                end
            else
                if dividendsB(t)>0
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(2)+permdbpes(1))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(2)+permdbpes(1))/(1/r_d(t));
                else
                    ydk(l)=(PA(W+F+l)/price(t)+V_divk(2)+permdbpes(2))/(1/r_d(t));
                    ydk_belief(l,t)=(V_divk(2)+permdbpes(2))/(1/r_d(t));
                end
            end
        end
        end
    ...
    if t>=shockperiod && switching==1
        ...
        permdkt=projectIncomeMult(prob_DD_kp(t),prob_DN_kp(t),prob_ND_kp(t),prob_NN_kp(t),pay1_true,pay2_true,r_d(t)); 
        end
        ...
        if dividends_income_k(l)>0
            if dividendsB(t)>0
                ydk_true=(V_divk_true(1)+permdbt(1))/(1/r_d(t));
            else
                ydk_true=(V_divk_true(1)+permdbt(2))/(1/r_d(t));
            end
        else
            if dividendsB(t)>0
                ydk_true=(V_divk_true(2)+permdbt(1))/(1/r_d(t));
            else
                ydk_true=(V_divk_true(2)+permdbt(2))/(1/r_d(t));
            end
        end
